...
**改动前必读架构（最高优先级强制）**：对任何模块的新增或修改，动代码之前必须先完整通读 [`.trae/docs/architecture.md`](../.trae/docs/architecture.md)（连同本文件、`CONTRIBUTING.md`、`AGENTS.md` 的文档链），用架构地图核实模块边界、数据流、调用链与锚点后，再结合代码与测试证实设计；禁止仅凭文件名或界面现象直接动手、跳读架构就写实现。
